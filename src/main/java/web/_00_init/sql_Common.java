...
public class sql_Common {
	...
	static String CREATE_TABLE_DEAL = "Create Table deal (" + "dealno int(7) NOT NULL Auto_Increment Primary Key, "
			...
			+ "dealimage MEDIUMBLOB, " + "dealfilename varchar(20), " + "goodsname varchar(20) NOT NULL,"
			+ "qty int(3) UNSIGNED NOT NULL," + "goodsimage MEDIUMBLOB," + "goodsimagename varchar(50),"
	...
	static String CREATE_FUNCTION_INSERT_DEAL = "CREATE FUNCTION insert_deal(USER_goodsno int,USER_INDID varchar(50),USER_QTY int,USER_ENDSHIPWAY int)"
			+ "RETURNS VARCHAR(10)BEGIN DECLARE function_qty INT DEFAULT 0;DECLARE function_goodstypes INT DEFAULT 0;DECLARE function_goodsname VARCHAR(10) DEFAULT '';"
			+ "DECLARE function_goodsloc INT DEFAULT 0;DECLARE function_goodsnote VARCHAR(200) DEFAULT '';DECLARE function_indid VARCHAR(50) DEFAULT '';DECLARE function_goodsimage MEDIUMBLOB;"
			+ "DECLARE result VARCHAR(10) DEFAULT '';SELECT QTY INTO function_qty FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT GOODSNAME INTO function_goodsname FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSTYPE INTO function_goodstypes FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT GOODSLOC INTO function_goodsloc FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSNOTE INTO function_goodsnote FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT INDID INTO function_indid FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSIMAGE INTO function_goodsimage FROM GOODS WHERE GOODSNO=USER_goodsno;IF function_qty >= USER_QTY THEN INSERT INTO DEAL VALUE("
			+ "NULL,NULL,function_indid,USER_INDID,0,USER_ENDSHIPWAY,USER_QTY,NULL,NULL,NULL,NULL,function_goodsname,QTY,function_goodsimage,CONCAT(USER_goodsno,CONCAT('_',UNIX_TIMESTAMP(NOW()))),"
			+ "function_goodstypes,function_goodsloc,function_goodsnote);UPDATE goods SET qty=(qty-USER_QTY) WHERE GOODSNO=USER_goodsno;SET result = 'TRUE';ELSE SET result = 'FLASE';END IF;RETURN result;END;";
