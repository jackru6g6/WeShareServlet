...
public class sql_Common {
	...
	static String CREATE_FUNCTION_INSERT_DEAL = "CREATE FUNCTION insert_deal(USER_goodsno int,USER_INDID varchar(50),USER_QTY int,USER_ENDSHIPWAY int,"
			+ "USER_DEALNO VARCHAR(200),USER_IMAGE MEDIUMBLOB,USER_FILENAME VARCHAR(20))RETURNS VARCHAR(100) BEGIN DECLARE function_qty INT DEFAULT 0;"
			...
			+ "DECLARE function_goodsstatus INT;DECLARE result VARCHAR(100) DEFAULT '';SELECT QTY INTO function_qty FROM GOODS WHERE GOODSNO=USER_goodsno;"
			...
			+ "SELECT GOODSstatus INTO function_goodsstatus FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "IF (function_qty >= USER_QTY) AND (USER_INDID != function_indid)THEN IF function_goodsstatus = 1 THEN "
			+ "INSERT INTO DEAL VALUE(NULL,NULL,USER_INDID,function_indid,0,USER_ENDSHIPWAY,USER_QTY,NULL,NULL,USER_DEALNO,USER_IMAGE,USER_FILENAME,"
			+ "function_goodsname,function_goodsimage,CONCAT(USER_goodsno,CONCAT('_',UNIX_TIMESTAMP(NOW()))),function_goodstypes,function_goodsloc,"
			+ "function_goodsnote,function_goodsstatus); ELSE "
			+ "INSERT INTO DEAL VALUE(NULL,NULL,function_indid,USER_INDID,0,USER_ENDSHIPWAY,USER_QTY,NULL,NULL,USER_DEALNO,USER_IMAGE,USER_FILENAME,"
			+ "function_goodsname,function_goodsimage,CONCAT(USER_goodsno,CONCAT('_',UNIX_TIMESTAMP(NOW()))),function_goodstypes,function_goodsloc,"
			+ "function_goodsnote,function_goodsstatus);END IF;	UPDATE goods SET qty=(qty-USER_QTY) WHERE GOODSNO=USER_goodsno; SET result = 'TRUE';"
			+ " ELSE SET result = 'FLASE';END IF;RETURN result;END;";
