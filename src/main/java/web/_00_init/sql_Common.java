...
public class sql_Common {
	...
	static String DROP_PROCEDURE_INSERT_FEEDBACK = "DROP PROCEDURE IF EXISTS INSERT_FEEDBACK";
	// -------------------------------------------------------------------------------------<CREATE>
	...
	static String CREATE_TABLE_FEEDBACK = "Create Table feedback("
			+ "dealno int(7) NOT NULL Auto_Increment Primary Key, "
	...
	static String CREATE_FUNCTION_INSERT_DEAL = "CREATE FUNCTION insert_deal(USER_goodsno int,USER_INDID varchar(50),USER_QTY int,"
			+ "USER_ENDSHIPWAY int,USER_DEALNO VARCHAR(200))RETURNS VARCHAR(10) BEGIN DECLARE function_qty INT DEFAULT 0;"
			+ "DECLARE function_goodstypes INT DEFAULT 0;DECLARE function_goodsname VARCHAR(10) DEFAULT '';DECLARE function_goodsloc INT DEFAULT 0;"
			+ "DECLARE function_goodsnote VARCHAR(200) DEFAULT '';DECLARE function_indid VARCHAR(50) DEFAULT '';DECLARE function_goodsimage MEDIUMBLOB;"
			+ "DECLARE function_goodsstatus INT;DECLARE result VARCHAR(10) DEFAULT '';SELECT QTY INTO function_qty FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSNAME INTO function_goodsname FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT GOODSTYPE INTO function_goodstypes FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSLOC INTO function_goodsloc FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT GOODSNOTE INTO function_goodsnote FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT INDID INTO function_indid FROM GOODS WHERE GOODSNO=USER_goodsno;SELECT GOODSIMAGE INTO function_goodsimage FROM GOODS WHERE GOODSNO=USER_goodsno;"
			+ "SELECT GOODSstatus INTO function_goodsstatus FROM GOODS WHERE GOODSNO=USER_goodsno;IF function_qty >= USER_QTY THEN IF function_goodsstatus = 1 THEN "
			+ "INSERT INTO DEAL VALUE(NULL,NULL,USER_INDID,function_indid,0,USER_ENDSHIPWAY,USER_QTY,NULL,NULL,USER_DEALNO,NULL,NULL,function_goodsname,"
			+ "function_goodsimage,CONCAT(USER_goodsno,CONCAT('_',UNIX_TIMESTAMP(NOW()))),function_goodstypes,function_goodsloc,function_goodsnote,function_goodsstatus);"
			+ "ELSE INSERT INTO DEAL VALUE(NULL,NULL,function_indid,USER_INDID,0,USER_ENDSHIPWAY,USER_QTY,NULL,NULL,USER_DEALNO,NULL,NULL,function_goodsname,function_goodsimage,"
			+ "CONCAT(USER_goodsno,CONCAT('_',UNIX_TIMESTAMP(NOW()))),function_goodstypes,function_goodsloc,function_goodsnote,function_goodsstatus);END IF;"
			+ "UPDATE goods SET qty=(qty-USER_QTY) WHERE GOODSNO=USER_goodsno;SET result = 'TRUE';ELSE SET result = 'FLASE';END IF;RETURN result;END;";
	...
	static String CREATE_FUNCTION_INSERT_FEEDBACK = "CREATE FUNCTION INSERT_FEEDBACK(USER_DEALNO int,USER_FBSOURCE VARCHAR(50),USER_FBENDID varchar(50),USER_FBTEXT VARCHAR(200),USER_FBSCORE INT(2),"
			+ "USER_FBIMAGE MEDIUMBLOB,USER_FBFILENAME VARCHAR(20))RETURNS VARCHAR(30) BEGIN DECLARE result VARCHAR(30) DEFAULT '';DECLARE buf0 INT DEFAULT 0;"
			+ "DECLARE buf1 INT DEFAULT 0;SELECT COUNT(*) INTO buf0 FROM DEAL WHERE (DEALNO=USER_DEALNO AND ENDID=USER_FBSOURCE AND dealstatus=2);"
			+ "IF buf0=1 THEN SET result = 'TRUE| '; INSERT INTO FEEDBACK VALUE(USER_DEALNO,null,USER_FBSOURCE,USER_FBENDID,USER_FBTEXT,USER_FBSCORE,USER_FBIMAGE,USER_FBFILENAME);"
			+ "ELSE SET result = 'FLASE|DEALNO NOT FOUND';END IF;RETURN result;END;";
	...
	static String CREATE_PROCEDURE_INSERT_FEEDBACK="CREATE PROCEDURE INSERT_FEEDBACK(IN USER_DEALNO int,IN USER_FBSOURCE VARCHAR(50),IN USER_FBENDID varchar(50),IN USER_FBTEXT VARCHAR(200),IN USER_FBSCORE INT(2),"
			+ "IN USER_FBIMAGE MEDIUMBLOB,IN USER_FBFILENAME VARCHAR(20))BEGIN DECLARE result VARCHAR(30) DEFAULT '';DECLARE buf0 INT DEFAULT 0;DECLARE buf1 INT DEFAULT 0;"
			+ "SELECT COUNT(*) INTO buf0 FROM DEAL WHERE (DEALNO=USER_DEALNO AND ENDID=USER_FBSOURCE AND dealstatus=2);IF buf0=1 THEN "
			+ "INSERT INTO FEEDBACK VALUE(USER_DEALNO,null,USER_FBSOURCE,USER_FBENDID,USER_FBTEXT,USER_FBSCORE,USER_FBIMAGE,USER_FBFILENAME);"
			+ "	SET result = 'TRUE| ';ELSE SET result = 'FLASE|DEALNO NOT FOUND';END IF;END;";
	// -------------------------------------------------------------------------------------<INSERT>
    ...
    static String CALL_FUNCTION_INSERT_FEEDBACK="{? =CALL INSERT_FEEDBACK(?,?,?,?,?,?,?)}";
