...
public class DealDAO {
	...
	public String OK_DEAL(String key,String INDID,String SHIPNO) {
		...
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con.prepareStatement("UPDATE DEAL SET DEALSTATUS=1 ,SHIPNO=? WHERE DEALNO =? AND SOURCEID=? ");) {
	...
	public String CANCEL_DEAL(String key,String INDID) {
		String ans = "FALSE";
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con.prepareStatement("UPDATE DEAL SET DEALSTATUS=2 WHERE DEALNO =? AND SOURCEID=? ");) {
	...
	public Collection<DEALBean> FindByENDKey_DEAL(String INDID) {
		...
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con
						.prepareStatement("SELECT * FROM DEAL WHERE ENDID=?  ORDER BY postdate DESC");) {
	...
	public Collection<DEALBean> FindBySOURCEKey_DEAL(String INDID) {
		...
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con
						.prepareStatement("SELECT * FROM DEAL WHERE SOURCEID=?  ORDER BY postdate DESC");) {
			pstmt.setString(1, INDID);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				DEALBean dealb = new DEALBean();
