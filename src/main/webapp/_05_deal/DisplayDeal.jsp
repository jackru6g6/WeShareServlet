...
<jsp:include page="/top.jsp" />
	...
	<c:if test="${!empty DEALSOURCE_DATA}">
		<h3>訂單(提供者)</h3>
		<table border='1'>
			<tr height='18' bgColor="${rowColor}">
				<td>交易編號</td>
				...
				<tr height='18' bgColor="${rowColor}">
					...
					<td><c:choose>
							<c:when test="${DEALSOURCE.DEALSTATUS==0}">
								處理中
								<Input type="button" name="cancel" value="取消" onClick="CancelDEAL(${DEALSOURCE.DEALNO})">
								<Input type="button" name="OK" value="結單" onClick="OKDEAL(${DEALSOURCE.DEALNO},${stVar.index})"> 
							</c:when>
					...
					<td><c:choose>
							<c:when test="${empty DEALSOURCE.SHIPNO}">
								<c:if
									...
									<input id="shipno${stVar.index}" type="text" name="SHIPNO" placeholder="物流編號 (可不輸入)">
								</c:if>
								<c:if
									test="${(DEALSOURCE.DEALSTATUS==0)&&(DEALSOURCE.ENDSHIPWAY==0)}">
									<input id="shipno${stVar.index}" type="hidden" name="SHIPNO" value="" >
								</c:if>
							</c:when>
